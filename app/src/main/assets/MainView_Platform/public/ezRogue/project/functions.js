var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	core.status.hero.hpmax = 9999999999999999999;
	core.status.hero.hp = core.getFlag("score");
	...
	core.waitHeroToStop(function () {
		...
		core.drawText([
			"\t[" + (reason || "恭喜通关") + "]你的分数是" + core.getStatus("hp") + "!"
		], function () {
	...
	hero_atk *= 1 + core.getFlag("atkboost") * 0.01;
	hero_def *= 1 + core.getFlag("defboost") * 0.01;
	...
	if (core.hasSpecial(mon_special, 3))
		mon_def = Math.max(mon_def, hero_atk - 1);
	// 如果是无敌属性，且勇士未持有十字架
	if (core.hasSpecial(mon_special, 20) && !core.hasItem("cross"))
		return null; // 不可战斗

	if (core.hasSpecial(mon_special, 29)) {
		mon_mdef = hero_mdef * enemy.value;
	}
	// 战前造成的额外伤害（可被护盾抵消）
	var init_damage = 0;
	...
	if (core.hasSpecial(mon_special, 30) && (hero_atk < mon_def))
		hero_per_damage = hero_atk;
	// 如果没有破防，则不可战斗
	if (hero_per_damage <= 0) return null;

	// 勇士的攻击回合数；为怪物生命除以每回合伤害向上取整
	var turn = Math.ceil((mon_hp + mon_mdef) / hero_per_damage);
	if (core.hasSpecial(mon_special, 34))
		turn += enemy.value;

	if (core.hasSpecial(mon_special, 32) && turn > enemy.n) {
		turn -= enemy.n;
		mon_atk *= (enemy.value || 1);
	}
	if (core.hasSpecial(mon_special, 33))
		turn += (per_damage * (turn - 1)) / hero_per_damage;
	// 魔攻：战斗伤害就是怪物攻击力
	if (core.hasSpecial(mon_special, 2) || (core.hasSpecial(mon_special, 30) && (hero_atk > mon_def)) || (core.hasSpecial(mon_special, 31) && (hero_def > mon_atk)) || ((mon_mdef > hero_atk) && core.hasSpecial(mon_special, 29)))
		per_damage = mon_atk;
	// 战斗伤害不能为负值
	...
	core.removeFlag("__extraTurn__");
	// ------ 支援END ------ //
	singledamage = per_damage * (turn - 1);
	if (core.hasSpecial(mon_special, 32))
		if (turn <= enemy.n)
			singledamage = 0;
	// 最终伤害：初始伤害 + 怪物对勇士造成的伤害 + 反击伤害
	var damage = init_damage + singledamage + turn * counterDamage;
	// 再扣去护盾
	damage -= hero_mdef;
